...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    # forword pass
    # (1)
    out_proj = np.dot(features, W_proj) + b_proj
    # (2)
    out_embed, cache_embed = word_embedding_forward(captions_in, W_embed)
    # (3)
    if self.cell_type == 'rnn':
      h, cache_rnn = rnn_forward(out_embed, out_proj, Wx, Wh, b)
    else:
      h, cache_rnn = lstm_forward(out_embed, out_proj, Wx, Wh, b)
    # (4)
    ...
    # backword pass
    # (4)
    dx_temp, dw_temp, db_temp = temporal_affine_backward(dx, cache_temp)
    # (3)
    if self.cell_type == 'rnn':
      dx_rnn, dh0_rnn, dWx_rnn, dWh_rnn, db_rnn = rnn_backward(dx_temp, cache_rnn)
    else:
      dx_rnn, dh0_rnn, dWx_rnn, dWh_rnn, db_rnn = lstm_backward(dx_temp, cache_rnn)

    # (2)
    dW_embed = word_embedding_backward(dx_rnn, cache_embed)
    # (1)
    dW_proj = np.dot(features.T, dh0_rnn)
    db_proj = np.sum(dh0_rnn, axis=0)
