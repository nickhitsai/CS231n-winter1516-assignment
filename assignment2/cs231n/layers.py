...
def affine_forward(x, w, b):
  ...
  #############################################################################
  _input = x.reshape(x.shape[0], -1)
  out = np.dot(_input, w) + b
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b)
...
def affine_backward(dout, cache):
  ...
  #############################################################################
  dx = np.dot(dout, w.T).reshape(x.shape) # (N, M) * (D, M).T = (N, D) and them reshape to the same shape as x
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (x, sample_mean, sample_var, x_norm, beta, gamma, eps)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  elif mode == 'test':
...
def batchnorm_backward(dout, cache):
  ...
  # page 4
  #
  (x, sample_mean, sample_var, x_norm, beta, gamma, eps) = cache
  N = dout.shape[0]

  dx_norm = dout*gamma
  dsample_var = np.sum((-1.0/2)*dx_norm*(x - sample_mean)*(sample_var + eps)**(-3.0/2), axis=0)
  dsample_mean = np.sum((dx_norm*(-1/np.sqrt(sample_var + eps))) , axis=0)
  dsample_mean += dsample_var*(-2/N)*np.sum(x - sample_mean, axis=0)
  dx = (1.0/np.sqrt(sample_var + eps))*dx_norm + (2.0/N)*dsample_var*(x-sample_mean) + dsample_mean/N
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  _x = np.lib.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)), 'constant')

  for n in xrange(N):
      for f in xrange(F):
          for _h in xrange(_H):
              for _w in xrange(_W):
                  out[n, f, _h, _w] = np.sum(_x[n, :, _h*stride:_h*stride + HH, _w*stride:_w*stride + WW] * w[f, :, :, :]) + b[f]
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in xrange(N):
    for c in xrange(C):
      for h in xrange(_H):
        for w in xrange(_W):
          tmp_matrix = x[n, c, h*stride:h*stride + pool_height, w*stride:w*stride + pool_width]
          index = np.unravel_index(tmp_matrix.argmax(), tmp_matrix.shape)
          dx[n, c, h*stride + index[0], w*stride + index[1]] = dout[n, c, h, w]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  # _in = x.reshape(N, C, -1)
  # for c in xrange(C):
  #   out[:, c, :], cache[c] = batchnorm_forward(_in[:, c, :], gamma[c], beta[c], bn_param)
  # out.reshape((N, C, H, W))
...
def spatial_batchnorm_backward(dout, cache):
  ...
  temp_output, dgamma, dbeta = batchnorm_backward(dout.transpose(0,3,2,1).reshape((N*H*W,C)), cache)
  dx = temp_output.reshape(N,W,H,C).transpose(0,3,2,1)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
